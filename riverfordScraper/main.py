...
ALLOWED_BOXES = ["Large veg box (less roots)", "Large veg box", "Bumper veg box (original)"]
...
def main():
    ...
    if cabbageCheck.containsCababage(allBoxesInfo[PREFERRED_BOX]):
        altBoxes = cabbageCheck.findAlternativeBoxes(allBoxesInfo, ALLOWED_BOXES)
        if len(altBoxes):
